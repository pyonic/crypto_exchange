...
const cryptoConverter = (
  ...
  const targetCurrency: Currency = prices.find(
    (price) => price.key === (to || DEFAULT_CURRENCY),
  );
  ...
  const currentPrice: number =
    currentCurrency.price * (amount || DEFAULT_CURRENCY_AMOUNT);
  const result: number = parseFloat(
    (currentPrice / targetCurrency.price).toFixed(10),
  );
